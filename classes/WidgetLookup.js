class WidgetLookup {
    ...
    fetchWidgetName(widgetId) {
        let widget = this.fetchWidget(widgetId);
    ...
    fetchWidget(widgetId) {
        ...
        if (/^\d+$/.test(widgetId)) {
            return this.allWidgets.find(w => w.widgetId == widgetId);
        } else {
            return widgetId == 'LastActorItem' ?
                this.allWidgets.find(w => w.type == widgetId) :
                this.allWidgets.find(w => w.widgetType == widgetId);
        }
    }
}
